...
public class Main7 {
    ...
    public static String specializationDeadines(int year){
        ...
        if (specializationList != null && specializationList.items != null) {
            DateTimeFormatter inputFormatter = DateTimeFormatter.ISO_DATE_TIME; //formátovač pro formát dat
            ...
            for (Specialization specialization : specializationList.items) {
                if(specialization.deadlinePrihlaska != null && specialization.deadlinePrihlaska.value != null){
                    try{
                        LocalDate date = LocalDate.parse(specialization.deadlinePrihlaska.value, inputFormatter); //parsovaní hodnoty uzávěrky do LocalDate
                        deadLines.add(date);
                    } catch (Exception e) {
